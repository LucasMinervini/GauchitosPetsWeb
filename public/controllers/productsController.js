...
const z = require('zod');
const { validateProd } = require('../public/schema/product.js'); 
const { ValidatePartialProd } = require('../public/schema/product.js');
...
const createProduct = (req, res) => {
  ...
  const categoryQuery = 'SELECT id FROM categorias WHERE id = ?';
...
const getCategory = (req, res) => {
  ...
  db.query('SELECT * FROM products WHERE category_id = ?', [category_id], (err, results) => {
    if (err) {
      console.log('Error executing query:', err);
      return res.status(500).send('Error executing query');
    }
